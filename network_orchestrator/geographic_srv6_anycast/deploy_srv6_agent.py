...
def _pid_map(workdir: Path) -> dict[str, str]:
    ...
    for line in pid_path.read_text(encoding="utf-8").splitlines():
        for entry in line.split():
            ...
            if parts[1] in assigned_pids:
                raise ValueError(f"duplicate namespace PID entry: {parts[1]}")
            mapping[parts[0]] = parts[1]
            assigned_pids.add(parts[1])
    if not mapping:
...
def deploy_agents(
    ...
    *,
    remote_id: int,
    agent_source: str | Path | None = None,
    agent_target: str | Path = (
        "/resources/controller/geographic_srv6_anycast/srv6_agent.py"
    ),
    python_source: str | Path | None = None,
    ...
    if not python_executable or not Path(python_executable).is_absolute():
        raise ValueError("python_executable must be an absolute target path")
    if startup_wait_s < 0:
    ...
    agent_target = Path(agent_target)
    python_source = Path(python_source or python_executable)
    if not agent_source.is_file():
        raise FileNotFoundError(
            f"SRv6 agent source does not exist: {agent_source}"
        )
    if not python_source.is_file():
        raise FileNotFoundError(
            f"Python interpreter source does not exist: {python_source}"
        )
    if not agent_target.is_absolute():
        raise ValueError("agent_target must be an absolute target path")
    processes = []
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--agent-target",
        default="/resources/controller/geographic_srv6_anycast/srv6_agent.py",
    )
