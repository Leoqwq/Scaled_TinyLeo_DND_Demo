// pyctr.c - A Python C extension for creating lightweight containers using Linux namespaces.
#include <Python.h>
...
static int container_init(
    ...
    int add_len = 50;
    char resources_dst[PATH_MAX+add_len];
    snprintf(resources_dst, sizeof(resources_dst), "%s/resources", newroot);
    if(mkdir(resources_dst, MODE) != 0) {
        return child_err("mkdir resources failed: ", err_fd);
    }

    char resources_src[PATH_MAX+add_len];
    snprintf(resources_src, sizeof(resources_src), "%s/resources", base_dir);
    if(mkdir(resources_src, MODE) != 0) {
        return child_err("mkdir resources failed: ", err_fd);
    }
    // mount --bind
    if(mount(resources_src, resources_dst, NULL, MS_BIND|MS_REC, NULL) != 0) {
        return child_err("mount --bind resources failed: ", err_fd);
    }

    // share controller dir
    char controller_dst[PATH_MAX+add_len];
    snprintf(controller_dst, sizeof(controller_dst), "%s/controller", resources_dst);
    if(mkdir(controller_dst, MODE) != 0) {
    ...
    // share /dev/shm
    // char dev[PATH_MAX];
    // snprintf(dev, sizeof(dev), "%s/dev", newroot);
    // if(mkdir(dev, MODE) != 0) {
    //     return child_err("mkdir dev failed: ", err_fd);
    // }
    char shm_dst[PATH_MAX+add_len];
    snprintf(shm_dst, sizeof(shm_dst), "%s/dev/shm", newroot);
    // if(mkdir(shm_dst, MODE) != 0) {
    //     return child_err("mkdir shm failed: ", err_fd);
    // }
    char shm_src[PATH_MAX+add_len];
    snprintf(shm_src, sizeof(shm_src), "/dev/shm");
    if(mount(shm_src, shm_dst, NULL, MS_BIND|MS_REC, NULL) != 0) {
        return child_err("mount --bind shm failed: ", err_fd);
    }
